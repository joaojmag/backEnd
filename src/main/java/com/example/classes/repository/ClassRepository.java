...
public interface ClassRepository extends JpaRepository<Class, Long> {
    // Declaração da interface ClassRepository que estende JpaRepository para
    // operações CRUD em Class.

    List<Class> findByTaught(String taught);
    // Método de busca personalizado que retorna uma lista de Class com base no
    // status "taught" (lecionada).

    List<Class> findByTitleContaining(String title);
    // Método de busca personalizado que retorna uma lista de Class com base no
    // título que contém uma sequência específica.
}