...
@CrossOrigin(origins = "*", maxAge = 3600)//(origins = "http://localhost:8081") // Permite solicitações de origens diferentes.
...
public class ClassController {
    ...
    public ResponseEntity<List<Class>> getAllClasses(@RequestParam(required = false) String title) {
        try {
            ...
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR); // Retorna erro interno do servidor em
                                                                                 // caso de exceção.
        }
    }
    ...
    public ResponseEntity<Class> createClass(@RequestBody Class classEntity) {
        try {
            Class newClass = classRepository
                    .save(new Class(classEntity.getTitle(), classEntity.getDescription(), classEntity.getTaught()));
            return new ResponseEntity<>(newClass, HttpStatus.CREATED); // Cria e retorna uma nova classe com status
                                                                       // "Criado".
        } catch (Exception e) {
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR); // Retorna erro interno do servidor em
                                                                                 // caso de exceção.
    ...
    public ResponseEntity<Class> updateClass(@PathVariable("id") long id, @RequestBody Class classEntity) {
        ...
        if (classData.isPresent()) {
            ...
            return new ResponseEntity<>(classRepository.save(existingClass), HttpStatus.OK); // Atualiza e retorna a
                                                                                             // classe com status OK.
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND); // Retorna "Não Encontrado" se a classe não existe.
        }
    }
    ...
    public ResponseEntity<HttpStatus> deleteClass(@PathVariable("id") long id) {
        try {
            ...
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR); // Retorna erro interno do servidor em caso
                                                                           // de exceção.
        }
    }
    ...
    public ResponseEntity<List<Class>> findByTaught() {
        try {
            ...
            if (taughtClasses.isEmpty()) {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT); // Retorna "Sem Conteúdo" se nenhuma classe
                                                                    // lecionada for encontrada.
            }
